...
async function main() {
  // Get input from command line arguments or use example
  const args = process.argv.slice(2);
  
  let courseInfo, task, vignette, outputNum;
  ...
  if (args.length >= 4) {
    ...
    console.log('Usage: node generate-vignette.js "<course>" "<task>" "<vignette>" <page_number>');
    console.log('\nExample:');
    console.log('node generate-vignette.js "A third-year database course" "Design a normalized database schema" "Student used AI to generate initial table structures..." 3');
    console.log('\nUsing default example...\n');
    
    // Default example
    courseInfo = "A third-year database systems course";
    task = "Students were asked to design a normalized relational database schema for an e-commerce platform";
    vignette = "In a third-year database course, a student was assigned to design a normalized database schema for an e-commerce system. The student used ChatGPT to generate an initial set of tables and relationships based on their requirements document. The AI produced a complete ER diagram with entities for Users, Products, Orders, and Reviews, including primary keys, foreign keys, and cardinality. The student then manually reviewed the schema, identified normalization issues, and refined the design to reach 3NF. The final deliverable included both the AI-generated initial schema and the student's refined version with annotations explaining the improvements.";
    outputNum = 3;
  }
  
  console.log('Generating vignette data...\n');
  console.log('Course:', courseInfo);
  console.log('Task:', task);
  console.log('Vignette:', vignette.substring(0, 100) + '...\n');
  
  try {
    const data = await generateVignetteData(courseInfo, task, vignette);
    
    console.log('✓ Data generated successfully\n');
    console.log('Scenario:', data.scenario_brief);
    console.log('AI Level:', data.slide5.ai_level);
    
    const html = generateHTML(data);
    
    const outputPath = path.join(process.cwd(), 'pages', `${outputNum}.html`);
    fs.writeFileSync(outputPath, html);
    
    console.log(`\n✅ Generated: ${outputPath}`);
    
    // Also save the JSON data for reference
    const jsonPath = path.join(process.cwd(), 'pages', `${outputNum}.json`);
